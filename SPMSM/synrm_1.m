classdef synrm_1 < handle
    ...
    properties
        ...
        Ns = 36; % number of stator slots % independent
        ...
        %stator dimensions
        ISD = 75.8; % inner stator diameter [mm] % indirect-dependent-editable
        OSD = 125; % outer stator diameter [mm] %..
        wst = 3; % width of stator tooth [mm] %..
        stta = 20; % tooth tip angle [deg] %indep
        bs0 = 2; % slot openning [mm] %..
        hs0 = 0.6; % slot openning height [mm]
        dss = 15; % depth of the stator slot [mm]
    ...
    methods
        function plotSketch(obj)
            ...
            [ux,uy] = rotate_pts(u2(1),u2(2), -pi/2);
            u3 = [ux, uy];

            r = fzero(@err_fcn, 1);

            [x4, y4] = getLineLineIntersection([0,-obj.wst*0.5/cos(obj.alpha_s/2)], u2, [obj.ISD/2+obj.dss-r,0], u3);
            [x5, y5] = deal(obj.ISD/2+obj.dss-r, 0);
            [x6, y6] = deal(obj.OSD/2, 0);

            [x7, y7] = deal(obj.ISD/2 * cos(obj.alpha_s/2), obj.ISD/2 * sin(obj.alpha_s/2));


            obj.x_spts = [x1 x2 x3 x4 x5 x6]; % 가이드 라인 플롯
            obj.y_spts = [y1 y2 y3 y4 y5 y6];


            t = linspace(0, atan2(y4-y5,x4-x5) ,20);
            x= x5 + r*cos(t);
            y= y5 + r*sin(t);
            xx = [x x4 x3 x2 x1 x7];
            yy = [y y4 y3 y2 y1 y7];
            ...
            plot(obj.OSD/2 *cos(tt), obj.OSD*sin(tt), 'b');
            plot(xx,yy, 'b');


            for i=1:obj.Ns
                [xr, yr] = rotate_pts(xx,yy,(i-1)*obj.alpha_s);
                plot(xr, yr, 'b');
            end


            function err = err_fcn(r)
                [xtmp, ytmp] = getLineLineIntersection([0,-obj.wst*0.5/cos(obj.alpha_s/2)], u2, [obj.ISD/2+obj.dss-r,0], u3);

                r2 = norm([xtmp, ytmp] - [obj.ISD/2+obj.dss-r,0]);
                err = r2-r;
            end
        end
